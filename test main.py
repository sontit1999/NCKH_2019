         ...
         for i in range(len(a)):
                for j in range(len(a[0])):
                    if(i!=0 and i!=len(a)-1 and j!=0 and j != len(a[0])-1):            
                        c[i][j] = math.atan(2*((a[i][j]-a[i-1][j-1])+(a[i][j]-a[i-1][j])+(a[i][j]-a[i-1][j+1])+(a[i][j]-a[i][j-1])+(a[i][j]-a[i][j+1])+(a[i][j]-a[i+1][j-1])+(a[i][j]-a[i+1][j])+(a[i][j])-a[i+1][j+1])/(a[i][j]))
#nomarlize
def nomalize(a):
